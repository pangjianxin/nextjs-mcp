...
export function useSignalRChat({ hubUrl }: UseSignalRChatOptions) {
  ...
  useEffect(() => {
    if (connection) {
      ...
      return () => {
        if (connection) {
          connection.off("ReceiveChat", handleReceiveChat);
        }
      };
    }
  }, [connection, handleReceiveChat]);
  ...
  const sendMessage = useCallback(
    async (inputMessage: string) => {
      ...
      try {
        // 调用SignalR的Chat方法（普通方法调用，不是流式）
        // Make sure the format matches what the server expects
        await connection.invoke("Chat", { input: inputMessage });

        // Alternative format if the above doesn't work:
        // await connection.invoke("Chat", { message: inputMessage });
      } catch (error) {
